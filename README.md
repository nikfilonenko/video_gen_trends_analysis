...
Для представления текста в векторной форме выбрана модель "Bag of Words" (BoW), так как LDA использует вероятностное моделирование на основе частот слов. Метод TF-IDF не применялся, чтобы избежать нарушения интерпретируемости тематических распределений.

#### Определение оптимального количества тем

Анализ проведён с использованием Coherence Score:

- При 3-10 темах Coherence Score очень низкий.
    
- Максимальный Coherence Score: **0.366 (43 темы)**.
    
- Оптимальный баланс между числом тем и интерпретируемостью: **29 тем (0.364)**.
    
- Разница между 43 и 29 темами по Coherence Score (0.002) минимальна, поэтому ключевой критерий – интерпретируемость.
    

Дополнительный эмпирический анализ:

- **8 тем**: сильное пересечение тем, низкая детализация, плохо сформированные кластеры.
    
- **43 темы**: повышенная детализация, но избыточная фрагментация схожих тем.
    
- **29 тем**: баланс между детализацией и читаемостью, ключевые тренды выражены чётко.
    

Вывод: **оптимальное число тем – 29**. Это обеспечивает высокую интерпретируемость без избыточного дробления.

#### Анализ выделенных трендов

LDA позволила выделить ключевые направления в тематике **генерации видео**. Анализ проводился двумя способами:

1. **Ручной анализ ключевых слов** – группировка тем в глобальные тренды.
    
2. **Автоматизированная кластеризация (BERT + UMAP + HDBSCAN)** – формирование трендов на основе эмбеддингов тем.

- Облако слов для кластера 1:

![img_1.png](assets/img_1.png)

- UMAP-визуализация кластеров

![img.png](assets/img.png)

**Основные тренды видеогенерации в 2024 году:**

- **Diffusion-модели для видео генерации** (темы: 1, 5, 6, 12, 21, 26, 29) – использование diffusion models для повышения качества видео.
    
- **Улучшение качества и редактирование видео** (темы: 5, 8, 25, 27) – генерация видео в высоком разрешении, HDR-обработка.
    
- **Распознавание и генерация движений в видео** (темы: 1, 6, 11, 16, 19) – анимация движений, распознавание действий.
    
- **Видео-аннотации и мультимодальный анализ** (темы: 2, 19, 28) – автоматическое описание видео, генерация видео по тексту.
    
- **NeRF и Light Field для рендеринга** (темы: 17, 18) – реалистичная генерация видеоконтента.
    
- **Применение видеогенерации в робототехнике** (темы: 10, 22, 24) – навигация, симуляция.
    
- **MedTech** (темы: 4, 9, 26) – анализ медицинских изображений (КТ, МРТ).
    

#### Сравнение подходов: LDA vs. кластеризация BERT + UMAP + HDBSCAN

- **LDA** – выделила **29 тем**, сгруппированных вручную в **7 глобальных трендов**.
    
- **LDA + кластеризация** – с применением **SBERT** для семантического объединения тем выявлено **5 ключевых трендов**, включая мультимодальность.
    

Результаты обоих подходов согласуются, что подтверждает надёжность выявленных закономерностей.

LDA позволила выделить основные тренды видеогенерации, которые были дополнительно подтверждены кластеризацией.

- Оптимальное число тем – **29**, что обеспечивает лучший баланс интерпретируемости и детализации. 
- Основные направления включают **diffusion-модели, HDR, мультимодальность, робототехнику и медицину**.
...
**Вывод топ-слов по значимости для n_components=10**
...
**Вывод топ-слов по значимости для n_components=19**
